 ...
 */
public class SequenceNodeParser<Result extends NodeResult, Context extends BaseContext> extends AbstractNodeParser<Result, Context> {

    public SequenceNodeParser(Node<Result> node, Class<?> resultClazz) {
        super(node, resultClazz);
        Preconditions.checkArgument(NodeType.Sequence.equals(node.getNodeType()), "Node type is not Sequence");
    ...
    public BehaviorNodeWrapper<Result, Context> parse() {
        ...
        return new BehaviorNodeWrapper<Result, Context>().buildSequenceNode(node.getNodeName(), children);
    }
}
