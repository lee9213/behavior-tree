 ...
 * @param <Context> 上下文类型
 */
public class RandomBuilder<Result extends NodeResult, Context extends BaseContext> extends NodeBuilder<Result, Context, RandomBuilder<Result, Context>> {
    private final List<NodeBuilder<Result, Context, ?>> childBuilders = new ArrayList<>();

    public RandomBuilder(BehaviorTree.Builder<Result, Context> treeBuilder, NodeBuilder<Result, Context, ?> parentBuilder, String nodeName) {
    ...
    public INode<Result, Context> buildNode() {
        ...
        return NodeFactory.createRandomNode(nodeName, childNodes);
    }
}
